class OrdersController < ApplicationController
  ...
  def create
    ...
    if @order.save
      $redis.publish "order-created", {user: @order.user.id, data: {order_id: @order.oid}}.to_json
  ...
  def cancel
    ...
    if @order
      if @order.update_attributes(status: "cancelled")
        @order.reload
        $redis.publish "order-cancelled", {user: @order.user.id, data: {order_id: @order.oid}}.to_json
